package
{
	...
	[SWF(width="1024", height="600", backgroundColor="#cccccc", frameRate="35")]
	...
	{		
		...
		*/
		private var erasableBitmapData:BitmapData = new BitmapData(WIDTH, HEIGHT, true, 0xFFFFFFFF);
		private var erasableBitmap:Bitmap = new Bitmap(erasableBitmapData);
		
		private var eraserClip:Sprite = new Sprite();		
		private var drawnBitmapData:BitmapData = new BitmapData(WIDTH, HEIGHT, true, 0x00000000);
		private var drawnBitmap:Bitmap = new Bitmap(drawnBitmapData);
		
		private var numPixels:int = WIDTH * HEIGHT;
		private var numPixelsAfterOffset:int = ((WIDTH/4) * (HEIGHT/4)) 
		...
		public function loadImages():void
		{
			...
			imageArray[imageArrayCount] = new TripToWonderlandImg();
			//imageArrayCount++;			
		}
		...
		{			
			stage.removeEventListener(MouseEvent.MOUSE_DOWN, initGame);
			...
			*/
			...
			stage.addEventListener(MouseEvent.MOUSE_OUT, maskOut);
			stage.addEventListener(MouseEvent.MOUSE_OVER, maskOver);			
		...
		{
			this.addChild(Menu);
			stage.addEventListener(MouseEvent.MOUSE_DOWN, initGame);
		}
		...
		private function updateAll(e:Event):void
		{
			if (frameCount >= 70) {
				...
				if (amountErased >= (numPixelsAfterOffset - (numPixelsAfterOffset * 0.2)))
				{
					reset();
				}
			} else { frameCount++; }
		...
		private function reset():void
		{
			...
			BottomImage = imageArray[Math.floor(Math.random() * imageArrayCount)];
